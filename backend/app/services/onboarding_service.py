...
from app.schemas.onboarding import ICPUpdate
...
class OnboardingService:
    ...
    async def create_or_update_icp(
        ...
        # Call Grok to generate ICP
        logger.info(f"Generating ICP for user {user_id} via Grok")
    ...
    async def update_icp(
        ...
        if not product_desc or not target_market:
            logger.error("Cannot regenerate ICP without product and target market descriptions")
            await self.db.commit()
